...
public class UnityMessageEvent : MonoBehaviour
{
    ...
    // 아래 순서는 호출 순서대로 적혀있음
    private void Awake()        // Start에 넣어야 하는 것을 여기다 넣으면 랜덤으로 터진다
    {
        // 가장 먼저 호출되는 함수
        ...
        //        (스크립트만의 초기화 작업)
        //		  (외부 게임상황과 무관한 초기화 작업) -> 외부 게임상황과 관련이 있는 작업을 진행하면 터진다
        // ex) 데이터 초기화, 컴포넌트 연결
        Debug.Log("Awake");
    }
    ...
    private void Start()
    {
        // 스크립트가 씬에 처음으로 Update하기 직전에(게임이 돌아가기 직전) 1회 호출됨
        // 스크립트가 비활성화된 경우 호출되지 않음

        // 역할 : 스크립트가 필요로 하는 초기화 작업 진행
        //        (다른 스크립트와의 연관 초기화 작업)
        //		  (외부 게임상황이 초기화에 영향을 줄 수 있는 작업)
        // ex) 몬스터의 플레이어 타겟선정
        Debug.Log("Start");
    }
